...
class trafficData(HttpUser):
    ...
    def sendMessage(self):
        ...
        if data is not None:
            print("Actualizado")
            res_grpc = self.client.post("/grpc/insert", json=data)
            res_rust = self.client.post("/rust/insert", json=data)
            #response = res.json()

            print("Respuesta de /rust/insert:", res_rust)
            print("Respuesta de /grpc/insert:", res_grpc)
